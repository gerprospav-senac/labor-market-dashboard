document.addEventListener("DOMContentLoaded", function(event) {
  ...
  const options = Array.from(reference?.options).map(item => item?.value);

  const value = sessionStorage.getItem('date-reference');
  if (value && options.includes(value)) {
    reference.value = value;
  } else {
    sessionStorage.setItem('date-reference', reference?.value);
  }

  reference.addEventListener('change', (event) => {
    sessionStorage.setItem('date-reference', event?.target?.value);
    document.dispatchEvent(new Event('onDateReferenceChange'));
  });
});
